...
ESTDesigner.Canvas = draw2d.Canvas.extend({
    ...
    this.on('contextmenu', function(emitter, event) {
      ...
      $.contextMenu({
        ...
        callback: $.proxy(function(key, options) {
          switch (key) {
            case 'Properties':
              emitter.contextmenuHandler(emitter)
              break
            default:
              break
          }
  ...
  getDefinitionsStartXML: function() {
    var xml = '<definitions xmlns="http://www.omg.org/spec/BPMN/20100524/MODEL"\n ' +
      ...
      'targetNamespace="' + (this.process.category == null ? '' : this.process.category) + '">\n'
    return xml
  },
  ...
  toXML: function() {
    ...
    xml = xml + '<process id="' + this.process.id + '" name="' + this.process.name + '" callBack="'+this.process.callBack +'">\n'
  ...
  toXML1: function() {
    ...
    for (var i = 0; i < models.getSize(); i++) {
      ...
      if (!isComposited) {
        let order = i
        this.nodeList.push(model.toXML1(order))
      }
    }
    var lines = this.getLines()
    for (var i = 0; i < lines.getSize(); i++) {
      ...
      if (!isComposited) {
        this.lineList.push(line.toXML1())
      }
    }
    this.task.lineList = this.lineList
    this.task.nodeList = this.nodeList
    this.task.taskId = this.process.id
    this.task.taskName = this.process.name
    this.task.callBack = this.process.callBack
    return this.task
  }
})
export default {
  render(id, width, height) {
    return new ESTDesigner.Canvas(id, width, height)
  }
}
