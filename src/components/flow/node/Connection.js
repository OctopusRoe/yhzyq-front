...
ESTDesigner.connection.Connection = draw2d.Connection.extend({
  init: function (attr) {
    this.line={"id":"","offset":"","to":"","name":"","condition":"","sourceId":""};
    this.name = null;
    this.callback = null;
    this.condition = null;
    this.sourceRef = null;
  ...
  toXML1: function (order) {
    ...
    this.line.sourceId=sourceId;
    this.line.name=lineName;
    this.line.to=targetId;
    this.line.condition=this.condition;
    return  this.line;
  },
  toBpmnDI: function () {
    var xml = '<bpmndi:BPMNEdge bpmnElement="' + this.getId() + '" id="BPMNEdge_' +
      this.getId() + '">\n';
    var startX = Math.round(this.getSource().getAbsoluteX());
    var startY = Math.round(this.getSource().getAbsoluteY());
    var endX = Math.round(this.getTarget().getAbsoluteX());
    var endY = Math.round(this.getTarget().getAbsoluteY());
    this.line.offset=startX+","+startY+","+endX+","+endY;
