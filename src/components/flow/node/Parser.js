...
ESTDesigner.tool.Parser.ProcessParser = ESTDesigner.tool.Parser.BaseParser.extend({
  ...
  parseProcess: function(xmlNode, process) {
    ...
    process.callBack = processNode.attr('callBack')
...
ESTDesigner.tool.Parser.UserTaskParser = ESTDesigner.tool.Parser.TaskParser.extend({
  ...
  _parseCustomizeProp: function(xmlNode, task) {
    ...
    let taskProposeInterceptor=xmlNode.attr("activiti:taskProposeInterceptor")
    let taskPostPositionInterceptor=xmlNode.attr("activiti:taskPostPositionInterceptor")
    if(taskProposeInterceptor){
      let data={
        interceptor:taskProposeInterceptor,
        taskId:task.id
      }
      task.taskProposeInterceptor = data
    }
    if(taskPostPositionInterceptor){
      let data={
        interceptor:taskPostPositionInterceptor,
        taskId:task.id
      }
      task.taskPostPositionInterceptor=data
    }
    if (dueDate != null && dueDate != '') {s
  ...
  _parseTaskPerformer: function(xmlNode, task) {
    ...
    if (performersExpression.indexOf('user(') != -1) {
      ...
      task.performerType = 'candidateGroups'
    } else if (performersExpression.indexOf('role(') != -1) {
      task.performerType = 'candidateRoles'
    }
    ...
    $.each(performers, function(i, n) {
      ...
      } else if (n.indexOf('role(') != -1) {
        start = 'role('.length
        var performer = n.substring(start, end)
        task.addCandidateRole(performer)
      }
    })
  },
