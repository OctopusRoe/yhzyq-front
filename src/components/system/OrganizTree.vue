...
<template>
  <div v-show="isShowTree">
    ...
    <el-tree
      ...
      node-key="key"
      :data="treeData"
      :expand-on-click-node="isExpandNode"
...
const generateList = (data, disabled) => {
  for (let i = 0; i < data.length; i++) {
    ...
    if (node.children) {
      ...
      generateList(node.children, disabled);
    }
  }
};
  ...
  methods: {
    ...
    async loadData() {
      ...
      await this.getTree({
        ...
        generateList(treeData, this.$props.isOnlySelectLeaf);
        ...
        if (this.treeTileData.length) {
          ...
          } else if (this.$props.isSelectFirstLeaf) {
            // Update：2021-6-10
            // Author：陈偲
          ...
          this.$nextTick(() => {
            this.$nextTick(() => {
              this.$refs.tree.setCurrentKey(key, true);

              this.$emit("select", this.$refs.tree.getCurrentNode(), node);
            });
          });
        }
      });
    ...
    // 复选框选中状态
    handlerCheck(data, isChecked) {
      if (isChecked) {
        this.checkedKeys.push(data.key);
      } else {
        const index = this.checkedKeys.indexOf(data.key);
