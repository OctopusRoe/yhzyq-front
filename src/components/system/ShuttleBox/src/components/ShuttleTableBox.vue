<template>
  <div class="shuttle-column">
    <!-- <h1>表格式穿梭框</h1> -->
    <p class="btns" v-if="$attrs.needClear">
      <el-button @click="handlerClearClick">清除</el-button>
    </p>
    <div class="transfer-colomn">
      <div class="transfer-colomn-left" :style="{ width: ltWidth }">
        ...
        >
          ...
          <el-table-column
            v-for="(info, index) in leftListHeader"
            :key="index"
            :prop="info.key"
        ...
        <el-pagination
          ...
          :pager-count="3"
          v-bind="$props.pageParams"
        >
        </el-pagination>
      </div>
      <div class="transfer-icons">
        <p @click="handleTransferRightClick">
          <i class="el-icon-arrow-right"></i>
        </p>
        <p class="to-left" @click="handleTransferLeftClick">
      ...
      <div class="transfer-colomn-right" :style="{ width: rtWidth }">
        ...
        >
          ...
          <el-table-column
            v-for="(info, index) in rightListHeader"
            :key="index"
            :prop="info.key"
            :label="info[tableShowName.label]"
            :min-width="info.width"
...
export default {
  ...
  watch: {
    ...
    clearedNums: {
      handler(val) {
        if (val && this.ifClearCheckedItem) {
          this.ClearHasCheckedItems();
        }
      },
      immediate: true
    },
    isShowDateList: {
      handler() {
        this.originDate.splice(0);
        this.originDate.push(...this.$attrs.isShowDateList);

        // this.originDate = this.$attrs.isShowDateList;
        setTimeout(() => {
          if (this.ifUseScroll) this.tableListScroll();
        });
      },
      deep: true
    }
  ...
  methods: {
    ...
    tableListScroll() {
      ...
      this.$refs.listTable.bodyWrapper.addEventListener(
        "scroll",
        res => {
    ...
    handleTransferRightClick() {
      ...
      this.originDate = arr;
      this.hasTransferedList = this.hasTransferedList.concat(
        this.checkedQuotaList
      );
      this.shuttledDate = this.objectArrayReset(
        this.hasTransferedList,
        this.rowKeyName
      );
    ...
    handleDbRebackRowClick(row) {
      ...
      if (this.shuttledDate.length) {
        this.shuttledDate.map((item, index) => {
          if (item[this.rowKeyName] === row[this.rowKeyName])
            rowCurIndex = index;
        });
