...
<script>
import { queryNewUser, saveUserRole } from "@/api/system/role";
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      ...
      console.log(ids);
      saveUserRole({ roleId: this.roleId, userIds: ids }).then((res) => {
        if (res.success) {
          this.$message({
            message: "关联成功",
            type: "success"
      ...
      // saveUserRole(this.req).then(response => {
